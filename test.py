...
#     # s3 = (2 * k, 3 * k)
#     # s4 = (3 * k, 4 * k + 1)
#     # p1 = Process(target=await get_student_data1, args=(*s1, q), daemon=True)
#     # p2 = Process(target=get_student_data1, args=(*s2, q), daemon=True)
#     # p3 = Process(target=get_student_data1, args=(*s3, q), daemon=True)
#     # p4 = Process(target=get_student_data1, args=(*s4, q), daemon=True)
#     # p1.start()
#     # p2.start()
#     # p3.start()
#     # p4.start()
...
#     # p3.join()
#     # p4.join()
#     # print((datetime.now() - st).total_seconds(), 'seconds')
#     # # st = datetime.now()
#     # # asyncio.run(extract_timetable_for_students_10_11(updating=True))
#     # # print((datetime.now() - st).total_seconds(), 'seconds')
#     # # df = pd.read_csv('timetables_csv/Акопян Э.csv')
#     # # print(df.to_string())
#     # # from py_scripts.funcs_teachers import extract_teacher_timetable_for_day, get_standard_timetable_with_edits_for_teacher
#     # # from py_scripts.funcs_students import extract_timetable_for_day, get_standard_timetable_with_edits_for_student
#     # #
#     # # print(asyncio.run(get_standard_timetable_with_edits_for_student('Пн', '10Б', 'Никита',
#     # #                                                                 'Муляр')))
#     # # print(asyncio.run(get_standard_timetable_with_edits_for_teacher('Пн', 'Э', 'Акопян')))
...
import pdfplumber
from py_scripts.consts import path_to_timetables, path_to_timetables_csv, \
    days_from_short_text_to_num, days_from_full_text_to_num
from multiprocessing import Process
...
if __name__ == "__main__":
    ...
    asyncio.run(extract_timetable_for_teachers(updating=True))
